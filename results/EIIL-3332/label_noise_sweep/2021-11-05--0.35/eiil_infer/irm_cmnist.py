...
def main(flags):
  ...
  for restart in range(flags.n_restarts):
    ...
    # Build environments
    envs = get_envs(flags=flags) # list of 4 dictionary
    print('input_env number of datasets')
    for env in envs:
      print(len(env['images']))
    # Define and instantiate the model
    ##if flags.color_based:  # use color-based reference classifier without trainable params
    ##  mlp_pre = ColorBasedClassifier()
    ##else:

    if flags.eiil or flags.eiil_infer:### 수정
      ...
      mlp = load_mlp(results_dir=None, flags=flags).cuda()
      mlp.train()


    # Define loss function helpers
    #def nll(logits, y, reduction='mean'):
    #  return nn.functional.binary_cross_entropy_with_logits(logits, y, reduction=reduction) ### CE 쓸 것이므로 삭제

    ### Negative Log Likelihood with cross entropy 추가
    def nll(logits, y, reduction='mean'):
    ...
    def mean_accuracy(logits, y):
      ...
      return ((y_pred - y) < 1e-2).float().mean()
    ### Multi-class classification을 위한 Accuracy // y_pred, y 모두 1d
    ...
    if flags.eiil or flags.eiil_infer: ### ERM Reference model을 학습시키는 과정
      ...
      envs = split_data_opt(envs, mlp_pre, flags) ####flags 추가
...
if __name__ == '__main__':
  ...
  parser.add_argument('--eiil_infer', action='store_true')
  ...
  parser.add_argument('--train_env_1__color_noise', type=float, default=0.1) ### 수정 0.2 -> 0.1
  parser.add_argument('--train_env_2__color_noise', type=float, default=0.2) ### 수정 0.1 -> 0.2
  parser.add_argument('--train_env_3__color_noise', type=float, default=0.3) ### 추가
