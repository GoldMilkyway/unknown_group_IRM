...
def get_envs(cuda=True, flags=None):
  ...
  def _make_environment(images, labels, e):
    ...
    ### 랜덤 Noise 발생기
    def make_noise(labels, noise, pure_label=3):
      import random
      """
      ...
      for idx in range(len(lab)):
        ...
        if noise[idx]:
          lab[idx] = random.choices(population = np.delete(noise_name, lab[idx].long()),
                                    weights = [0.2,0.8],
                                    k = 1)[0]
      return lab
    ###
    ...
    for i in range(len(labels)):
      if labels[i] < 3:
        labels[i] = 0     # 0,1,2 - label: 0
      elif labels[i] > 6:
        labels[i] = 2     # 7,8,9 - label:2
      else:
        labels[i] = 1     # 3,4,5,6 - label: 1
    ###
    ...
    images = torch.stack([images, images, images], dim=1) ### stack 2개 --> 3개 (2color -> 3color)
  ...
  mnist_train = (mnist.data[:50001], mnist.targets[:50001]) ### 50000->50001 3env로 나누어 떨어지기 위함
  mnist_val = (mnist.data[50001:], mnist.targets[50001:])  ### 10000->9999 3env로 나누어 떨어지기 위함
...
#    _make_environment(mnist_train[0][30000:45000], mnist_train[1][30000:45000], flags.train_env_2__color_noise),
#    _make_environment(mnist_train[0][45000:], mnist_train[1][45000:], flags.train_env_3__color_noise),
#    _make_environment(mnist_val[0], mnist_val[1], flags.test_env__color_noise)
#  ]
  envs = [
    _make_environment(mnist_train[0][::3], mnist_train[1][::3], flags.train_env_1__color_noise),
    _make_environment(mnist_train[0][1::3], mnist_train[1][1::3], flags.train_env_2__color_noise),
    _make_environment(mnist_train[0][2::3], mnist_train[1][2::3], flags.train_env_3__color_noise),
    _make_environment(mnist_val[0], mnist_val[1], flags.test_env__color_noise)
  ]
